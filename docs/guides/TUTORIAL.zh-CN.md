...
1. 打开设置（右上角齿轮）。
2. 如果当前还没完成模型配置。
   - 应用不会自动启动本地 llama.cpp 或远�?runtime。
   - 搜索框会保持禁用。
   - 搜索框位置会显示红色内联提示：`未配置模型，请在 设置 > 模型 中配置`
3. �?**基础** 页选择监听目录�?Top-K。
4. �?**模型** 页选择 **本地 llama.cpp** �?**远程 API**，并填写 endpoint / key / 三角色模型。
...
- 统计一�?0：检查目录是否有效、索引是否暂停、是否需要手动重建。
...
Memori-Vault 当前架构定位�?**Local-first Verifiable Memory OS Lite**，详细设计见 [MEMORY_OS_LITE.md](../architecture/MEMORY_OS_LITE.md)。

使用时请重点关注。

- 答案区不只看正文，还要看 citation、evidence、Trust Panel �?retrieval metrics。
- Trust Panel 会展�?`answer_source_mix`、`failure_class`、`source_groups`、`memory_context` �?`context_budget_report`。
- 对话/项目记忆可以帮助回答，但只能作为 `memory_context`，不能冒充文�?citation。
- 设置页的 Memory 选项用于控制 conversation memory、auto memory write、source requirement �?context budget；Markdown export 仍是计划中能力，当前保持关闭。
- MCP endpoint 默认�?`http://127.0.0.1:3757/mcp`，agent 可通过 `ask/search/get_source/open_source` �?`memory_search/memory_add/memory_update` 等工具调用本地知识库。
